...
export default function DoctorAppointmentsClient() {
  ...
  async function load() {
    ...
    const response = await fetch(`/api/doctor/appointments?${query.toString()}`, { cache: "no-store" });
    setLoading(false);
    if (!response.ok) {
  ...
  async function updateStatus(id: string, newStatus: string) {
    ...
    if (!response.ok) {
      ...
      showToast({ type: "error", title: "Update failed", description: error.error });
      return;
    }
            ...
            <tr key={appointment.id}>
              ...
              <td style={{ display: "flex", gap: "0.5rem" }}>
                {appointment.status === "PENDING" ? (
                  <>
                    <button className="button" onClick={() => updateStatus(appointment.id, "APPROVED")}>Approve</button>
                    <button className="button danger" onClick={() => updateStatus(appointment.id, "REJECTED")}>Reject</button>
                  </>
                ) : null}
                {appointment.status === "APPROVED" ? (
                  <button className="button" onClick={() => updateStatus(appointment.id, "COMPLETED")}>
                    Mark Completed
                  </button>
                ) : null}
              </td>
