...
import { adminCreateAppointmentSchema, appointmentFiltersSchema } from "@/lib/validation";
...
export async function GET(request: Request) {
  ...
  if (filters.doctorId) where.AND.push({ doctorId: filters.doctorId });
  if (filters.patientId) where.AND.push({ patientId: filters.patientId });
...
export async function POST(request: Request) {
  ...
  const { patientId, doctorId, date, time, symptoms, status } = parsed.data;
